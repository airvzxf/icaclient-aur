...
- **`bundle-4.0-icu70`** (not started) — D.3 variant, the most ambitious candidate. Extracts `Webkit2gtk-4.0` from the Citrix tarball, runs `patchelf` on the Citrix binaries and the WebKit helpers, string-patches the hardcoded `/usr/lib/x86_64-linux-gnu/webkit2gtk-4.0/...` paths. Implementation plan in [`docs/d3-bundle-implementation-plan.md`](docs/d3-bundle-implementation-plan.md). Will be worked on in a future session.
